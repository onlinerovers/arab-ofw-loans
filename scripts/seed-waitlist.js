/**
 * Seed 30 dummy Gulf-region applicants per day for 7 days.
 * Run: node scripts/seed-waitlist.js
 */
...
function randomName() {
  const first = Math.random() > 0.35 ? pick(FIRST_NAMES_MALE) : pick(FIRST_NAMES_FEMALE);
  return `${first} ${pick(LAST_NAMES)}`;
}
function randomEmail(name) {
  const clean = name.toLowerCase().replace(/[^a-z]/g, '.').replace(/\.+/g, '.');
  return `${clean}${rand(1, 999)}@${pick(['gmail.com','yahoo.com','hotmail.com','outlook.com'])}`;
}
function randomPhone() {
  return `${pick(['+965','+971','+966','+974','+973','+968'])}${rand(50000000, 99999999)}`;
}
